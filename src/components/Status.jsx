...
const Status = () => {
  ...
  const handleSchemeSubmit = async (e) => {
    ...
    try {
      await axios
        .post(
          "https://dgs-backend-yo9v.onrender.com/api/v1/user/scheme/checkSchemeStatus",
          { registration_no: scheme }
        )
        .then((res) => {
          console.log(res);
  ...
  return (
    <div className="flex flex-col items-center h-screen pt-10 mt-5 lg:pt-0">
      ...
      {resultScheme && (
        <div>
          {resultScheme.initial_status === "pending" &&
          resultScheme.finial_status === "pending" ? (
            <p className="mt-4 text-green-700">
              Your initial status is {resultScheme.initial_status}
            </p>
          ) : (
            <p className="mt-4 text-green-700">
              {" "}
              Your initial status is {resultScheme.initial_status} . Your final
              status is {resultScheme.final_status}
            </p>
          )}
        </div>
      )}
