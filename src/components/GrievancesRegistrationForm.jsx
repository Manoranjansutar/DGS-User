  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        "https://dgs-backend-yo9v.onrender.com/api/v1/grievances/apply",
        formData,
        {
          headers: {
      ...
      if (response.data.message === "Grievances applied successfully") {
        ...
        navigate("/grievances_success", {
          state: { grievance: response.data.data },
        });
      }
  ...
  return isAuthenticated ? (
    ...
    <div className="h-[70vh]">
      ...
      <p className="my-2 text-center">
        If you are a registered user, then login to DGS by providing Username
        and Password in User{" "}
        <Link to="/userLogin" className="font-bold text-green-900 ">
          Login
        </Link>{" "}
        section.
      </p>
